...
The current package is a reproducibility draft aligned with the thesis v17 terminology and terminology ledger. The canonical naming rules are recorded in `docs/terminology_ledger_20260614.md`. The cleaned run summary contains 2,439 run-level records, including 1,769 completed runs and 990 runs marked `usedForScenarioStats=true`. The scenario-statistics subset includes the full coupled SEIRG model (`modelMode=0`), the 30-seed budget-matched pathway-fairness ABM check, and three baseline structures: traditional SEIR (`modelMode=1`), single-layer network SEIR (`modelMode=2`), and no-G/no-coupling model (`modelMode=3`). The M1-M3 baseline outputs are included for comparison and should be rechecked after any future AnyLogic model edit.
...
Because AnyLogic is proprietary software, the package also includes `docs/seirg_transition_specification.md`, `scripts/analysis/mean_field_reference_seirg.py`, `scripts/analysis/mean_field_pathway_budget_check.py`, and `scripts/analysis/build_pathway_fairness_abm_check.py`. These files provide compact transition-equation documentation, a standard-library SEIRG sanity check for state-flow direction, an equation-level allocation check, and a completed 30-seed budget-matched ABM pathway-fairness table. The primary manuscript simulation results remain the documented AnyLogic exports and processed summaries.
...
python3 scripts/analysis/build_pathway_fairness_abm_check.py
