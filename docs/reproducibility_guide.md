...
8. Inspect `docs/seirg_transition_specification.md` for the compact SEIRG transition-equation documentation.
9. Run `python3 scripts/analysis/mean_field_reference_seirg.py` for a standard-library equation-level SEIRG sanity check. This check is provided for transparency and does not replace the AnyLogic agent-based simulation runs used in the manuscript.
10. Install `requirements.txt`, then run `python3 scripts/analysis/build_chapter5_nine_figures.py` to regenerate the Chapter 5 mean-curve figures in `results/figures/第五章九情景重绘/`.
11. Compare the generated tables and figures with the manuscript.
